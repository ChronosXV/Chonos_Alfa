...
namespace MenuInicio
{
    ...
    //falta hacer algunas validaciones, si texbox esta vacio, que cuando consulte si no existe que no muestre nada...
    public partial class A_1_11_RegistrarPerfil : Form
    {
        public A_1_11_RegistrarPerfil()
        {
            InitializeComponent();
        }
        SqlConnection conex;
        ...
        private void A_1_11_RegistrarPerfil_Load(object sender, EventArgs e)
        {
        ...
        {
            comando = new SqlCommand("insert into chronos.dbo.perfiles (descripcion) values (@descripcion)", conex);
            SqlCommand consulta = new SqlCommand("select count(descripcion) from chronos.dbo.perfiles where descripcion=@des", conex);
            ...
            try
            {
                if (textBox1.Text == "")
                {
                    MessageBox.Show("Ingrese un Perfil por favor", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    conex.Open();
                    ...
                    if (n >= 1)
                    {
                        MessageBox.Show("El Perfil ya Existe, verifique la informacion ingresada", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        textBox1.Text = "";
                    }
                    else
                    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                conex.Open();
                SqlCommand select = new SqlCommand("SELECT descripcion as Perfiles  FROM chronos.dbo.perfiles ", conex);
                adaptador.SelectCommand = select;
                data = new DataSet();
                adaptador.Fill(data, "Perfiles");
                dataGridView1.DataSource = data;
                dataGridView1.DataMember = "Perfiles";
        ...
        {
            SqlCommand eliminar = new SqlCommand("DELETE FROM Chronos.dbo.perfiles WHERE descripcion=@Descrip", conex);
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("El Perfil no Existe", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        textBox1.Text = "";

                    }
                    else
                    {
                        MessageBox.Show("El Perfil seleccionado se ha eliminado", "Informacion", MessageBoxButtons.OK);
                        textBox1.Text = "";
                    }
                }
            }
        ...
        {
            SqlCommand actualizar = new SqlCommand(" UPDATE Chronos.dbo.perfiles SET descripcion=@Descrip1 where descripcion=@Descrip", conex);
            ...
            try
            {
                ...
                else if (textBox2.Text == "") { MessageBox.Show("Ingrese un Perfil a modificadar por favor", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
                else
                {
        ...
        private void A_1_11_RegistrarPerfil_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
    }
