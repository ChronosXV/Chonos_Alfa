...
{
    public partial class A_1_8_RegistrarPuesto : Form
    {
        public A_1_8_RegistrarPuesto()
        {
            InitializeComponent();
        }
        SqlConnection conex;
        ...
        private void A_1_8_RegistrarPuesto_Load(object sender, EventArgs e)
        {
        ...
        {
            comando = new SqlCommand("insert into chronos.dbo.puestos (descripcion) values (@descripcion)", conex);
            SqlCommand consulta = new SqlCommand("select count(descripcion) from chronos.dbo.puestos where descripcion=@des", conex);
            ...
            try
            {
                if (textBox1.Text == "")
                {
                    MessageBox.Show("Ingrese un Puesto por favor", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    conex.Open();
                    ...
                    if (n >= 1)
                    {
                        MessageBox.Show("El Puesto ya Existe, verifique la informacion ingresada", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        textBox1.Text = "";
                    }
                    else
                    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                conex.Open();
                SqlCommand select = new SqlCommand("SELECT descripcion as PUESTOS FROM chronos.dbo.puestos ", conex);
                adaptador.SelectCommand = select;
                data = new DataSet();
                adaptador.Fill(data, "Puestos");
                dataGridView1.DataSource = data;
                dataGridView1.DataMember = "Puestos";
        ...
        {
            SqlCommand eliminar = new SqlCommand("DELETE FROM Chronos.dbo.puestos WHERE descripcion=@Descrip", conex);
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("El Puesto no Existe", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        textBox1.Text = "";
        ...
        {
            SqlCommand actualizar = new SqlCommand(" UPDATE Chronos.dbo.puestos SET descripcion=@Descrip1 where descripcion=@Descrip", conex);
            ...
            try
            {
                ...
                else if (textBox2.Text == "") { MessageBox.Show("Ingrese el Puesto a modificada por favor", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
                else
                {
        ...
        private void A_1_8_RegistrarPuesto_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
    }
